...
const ImageSlider: React.FC<ImageSliderProps> = () => {
  ...
  const handleCardClick = (image: string) => {
    setSelectedImage(process.env.NEXT_PUBLIC_MODE == "dev" ? process.env.NEXT_PUBLIC_API_IMAGE_STAGING : process.env.NEXT_PUBLIC_API_IMAGE_PROD + image);
  };

  useEffect(() => {
    if (dataGetTopNews !== undefined) {
      setSelectedImage(
        process.env.NEXT_PUBLIC_MODE == "dev" ? process.env.NEXT_PUBLIC_API_IMAGE_STAGING : process.env.NEXT_PUBLIC_API_IMAGE_PROD + dataGetTopNews[0].image
      );
    }
  }, [dataGetTopNews]);
